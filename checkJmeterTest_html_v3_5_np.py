...
def calculate_metrics(jtl_df, transaction_name, expected_tps, expected_error_rate, expected_avg, expected_90p, expected_99p):
    ...
    # Calculate average, 90th, and 99th percentiles
...
<head>
    ...
    <style>
        ...
        .container {{
            max-width: 95%;  /* Changed from 1200px to be more responsive */
            margin: 0 auto;
            background-color: white;
            padding: 20px;
            box-shadow: 0 0 10px rgba(0,0,0,0.1);
            border-radius: 8px;
            overflow-x: auto;  /* Added to handle horizontal scroll if needed */
        }}
        h1 {{
            color: #2c3e50;
            text-align: center;
        ...
        table {{
            ...
            min-width: 1200px;    /* Added to ensure minimum width */
        }}
        th, td {{
        ...
        th {{
            background-color: #f8f9fa;  /* Added header background */
            font-weight: 600;
        }}
        .status-passed {{
            ...
            min-width: 70px;       /* Added minimum width for status */
        }}
        .status-failed {{
