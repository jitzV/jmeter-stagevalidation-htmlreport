# filepath: /JMeterReportProject/jmeter_report/script.py
import os
import sys
import pandas as pd
import numpy as np
...
class JMeterReport:
    def __init__(self, jtl_file_dir, jtl_threshold):
        self.jtl_file_dir = jtl_file_dir
        self.jtl_threshold = jtl_threshold
        self.pipeline_result = 0

    def validate_metrics(self, transaction_name, env_vars):
    ...
    def calculate_metrics(self, jtl_df, transaction_name, expected_tps, expected_error_rate, expected_avg, expected_90p, expected_99p):
        ...
        percentile_90 = self.jmeter_percentile(transaction_data['elapsed'], 90)
        percentile_99 = self.jmeter_percentile(transaction_data['elapsed'], 99)
    ...
    def generate_report(self):
        if not os.path.exists(self.jtl_file_dir):
            logging.error(f"Error: Could not find directory {self.jtl_file_dir}")
            sys.exit(1)

        jtl_files = [f for f in os.listdir(self.jtl_file_dir) if f.endswith('.jtl')]

        if not jtl_files:
            logging.error(f"Error: No JTL files found in directory {self.jtl_file_dir}")
            sys.exit(1)

        for jtl_file_name in jtl_files:
            jtl_file_path = os.path.join(self.jtl_file_dir, jtl_file_name)
            ...
            formatted_duration = self.format_duration(overall_duration_seconds)
            overall_threads = len(jtl_df['threadName'].unique()) if 'threadName' in jtl_df.columns else 0

            http_code_counts = self.get_http_code_counts(jtl_df)
            http_code_html = ""
            if http_code_counts:
            ...
            for var_name, var_value in os.environ.items():
                if var_name.lower().endswith('_tps'):
                    ...
                    if not self.validate_metrics(transaction_name, os.environ):
                        continue
                    ...
                    tps, error_rate, total_transactions, average, percentile_90, percentile_99 = self.calculate_metrics(
                        jtl_df, transaction_name, expected_tps, expected_error_rate, expected_avg, expected_90p, expected_99p
                    )

                    avg_status = self.is_within_threshold(average, expected_avg, self.jtl_threshold)
                    p90_status = self.is_within_threshold(percentile_90, expected_90p, self.jtl_threshold)
                    p99_status = self.is_within_threshold(percentile_99, expected_99p, self.jtl_threshold)
            ...
            report_file_path = os.path.join(self.jtl_file_dir, f"{os.path.splitext(jtl_file_name)[0]}_report.html")
            with open(report_file_path, "w") as report_file:
                report_file.write(html_content)

            logging.info(f"Report generated at {report_file_path}")

        sys.exit(self.pipeline_result)
...
def main():
    ...
    report = JMeterReport(jtl_file_dir, jtl_threshold)
